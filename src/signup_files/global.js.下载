...
//弹出确认框
//例如:
//1、KnowfarData.Confirm("是否操作", act, null) //函数不加()
//2、KnowfarData.Confirm("是否操作", "alert('yes')", "alert('no')")
/////////////////////////////
                ...
                if (d.result == "1") {//成功登录
                    ...
                    var _ajaxGetLoginbar = '\
						<ul>\
							...
							<li class="ico points"><a href="' + site.Dir + 'user/bobi_buypoints.aspx" target="_blank" title="当前账户剩余' + d.points + '元，点击马上充值">账户</a></li>' + _pmBar + _cartBar + '\
						</ul>\
					';
...
/*当前网站访问者信息*/
var user = new Object();
if(KnowfarData.Cookie.get(site.CookiePrev + "user", "id") == ""){//游客
    user.id = "0";
    user.name = "guest";
    user.userkey = "666666";
    user.groupid = "0";
